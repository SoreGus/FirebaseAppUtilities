...
def build_parser() -> argparse.ArgumentParser:
    ...
    status = subparsers.add_parser("status", help="Show the connected project configuration")
    _add_config_argument(status)

    collections = subparsers.add_parser("collections", help="List Firestore collections")
    _add_config_argument(collections)

    documents = subparsers.add_parser("documents", help="List documents from a collection")
    documents.add_argument("collection")
    documents.add_argument("--limit", type=int, default=50)
    _add_config_argument(documents)

    analytics = subparsers.add_parser("analytics", help="Query custom analytics events")
    ...
    function = subparsers.add_parser("function", help="Call an HTTP function endpoint")
    ...
    gui = subparsers.add_parser("gui", help="Open the local GUI")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "analytics":
            if args.counts:
                _print_json(project.analytics.count_by_name(days=args.days))
            else:
                _print_json(
        ...
        if args.command == "function":
            payload = json.loads(args.json_payload) if args.json_payload else None
